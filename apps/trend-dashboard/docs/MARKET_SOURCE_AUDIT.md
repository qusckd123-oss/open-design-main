...
### Rednape `/category/accessories/45/` composition finding (2026-09-15, pre-implementation audit)

Before any collector was written, a full enumeration of this category's live product names found that **the page's content does not match the `BAG` mapping 1:1** - this is a category-composition fact about the source page, not a defect in the already-committed config (see below).

- **22 products currently listed** on `/category/accessories/45/` (confirmed via raw HTML, all `anchorBoxId_{id}` entries on page 1; `?page=2` returns zero products, i.e. this category has only one page today).
- **Only 5 of the 22 are confirmed bags**: `593` (아카이브 나일론 에코백), `586` (더로 레디 빅 쇼퍼백), `509` (리플렉티브 테크니컬 나일론 로프백), `440` (토고 쉘 경량 그리드 백팩), `45` (스카치 멀티 미니 크로스백). The remaining 17 are shoes/loafers (`707`, `465`), caps (`591`, `480`, `325`, `289`), belts (`452`, `443`, `115`, `75`, `72`), mufflers/scarves (`418`, `358`, `184`, `148`), a beanie (`675`), and gloves (`298`).
- **Confirmed current bag-name suffixes** (observed directly in the 5 passing product names above, not invented or broadened beyond what's present today): `에코백`, `쇼퍼백`, `로프백`, `백팩`, `크로스백`.
- **A future Rednape BAG collector MUST apply a product-name gate using these confirmed terms before emitting any `BAG` row.** It must NOT collect this category wholesale (i.e. must not emit all 22 products as `BAG`). Products that fail the gate must be **skipped entirely** - never emitted as `BAG`, and never forced into a `subItemType: "OTHER"` bucket while still counting as a `BAG` row. This repo's existing category-refinement layer (`classifyMarketAttributes`/`refineObservedCategory`/`bagSubItem` in `src/collectors/market/classification.ts`, shared with END/Rakuten) was checked and confirmed to have **no rejection mechanism** for this - it only ever refines sleeve length or picks a bag sub-type, it never excludes a wrong-category product. The gate must therefore live in the not-yet-written Rednape-specific collector code, not in that shared file.
- **The committed config remains valid as-is.** `sourceCategoryConfigs.REDNAPE`'s `BAG: "/category/accessories/45/"` mapping describes a real page that does contain real bag products - it never claimed every product on that page is a bag. The filtering requirement above is a **collection-behavior** requirement for the collector that doesn't exist yet, not a config correction.
- **Zero Rednape rows have been collected.** This remains a documentation-only finding from a read-only audit; no collector was written or run.
