...
## P1-2 Product UX - Selected Signal Visual Evidence Hierarchy (2026-09-14, approved and implemented)

Rebalances `SelectedSignalDetail`'s internal content order and `EditorialVisualContextStrip`'s presentation so interpretation is always encountered before article imagery, and `ARTICLE_HERO` images read as supporting editorial context rather than a dominant, hero-like evidence block. Presentation-only: no ranking, bundle ordering, Watchlist selection/interaction, FACT/UNKNOWN/PLANNING QUESTION semantics, coverage/momentum semantics, collector, Prisma, schema, DB, taxonomy, Visual Diffusion data, or Market/store logic changed.

- **Content order (was image-strip-then-interpretation, now interpretation-then-image-strip)**: `SelectedSignalDetail` now renders eyebrow -> heading -> (dormant direct/adjacent-relation hero image, unchanged, a strictly stronger evidence tier than article context) -> 관측 강도/최근 방향/최신 관측 -> FACT -> UNKNOWN -> PLANNING QUESTION -> `기사 비주얼 맥락` -> Visual Diffusion lane -> 근거 보기. A planner now reaches the full interpretation block before any article imagery on both breakpoints.
- **`ARTICLE_HERO` treatment reduced from dominant to supporting**: `EditorialVisualContextStrip` moved from up to 6 large (aspect 4:5) images in a 3-5 column grid with a text-signal-accented heading and a gradient publisher/date overlay on each image, to small (square, ~80-96px) muted-toned thumbnails with a plain caption below each image, positioned after - not beside/above - the interpretation block. The persistent disclaimer sentence ("기사 대표 이미지 · 아이템/속성/무드를 직접 증명하지 않음") remains visible text at all times, never a title-only/hover-only tooltip.
- **`SelectedSignalDetail` explicitly limits to 3 images; the shared component's own default is unchanged at 6**: `EditorialVisualContextStrip` gained an optional `limit` prop, but its default stays `6` (the same value it always effectively used) so no other/future consumer is silently affected. Only the `SelectedSignalDetail` call site passes `limit={3}` for its own restrained treatment - a caller-site choice, not a shared-component behavior change. This component's only consumer today is `SelectedSignalDetail` (verified before implementing).
- **`ARTICLE_HERO` remains editorial context, never direct evidence**: unchanged - `BundleHeroImage` (the DIRECT_BLOCK/ADJACENT_BLOCK-gated, document-position-confident image) remains the only image type ever treated as evidence-adjacent, and remains dormant/null for every real bundle today, exactly as before.
- **Visual Diffusion**: unchanged - still renders nothing, still the empty `visualDiffusionReferences` array, still positioned after `기사 비주얼 맥락` in the content order.

Files touched: `src/components/AttributeBundle.tsx` only (one file, ~120 lines changed). No other file changed.
...
- `src/config/market-sources.ts`: added `"REDNAPE"` to `marketSources` and a matching `normalizeMarketSource` branch.
- `src/config/market-category-map.ts`: added a new `CAFE24_CATEGORY_HTML` literal to `SourceCategoryConfig.method` (narrower/truthful for a non-Shopify, non-ranking-page HTML source - not a guess at "any HTML site"), and a `REDNAPE` entry (`rankingVerified: false`, `rankingScope: "CATEGORY"`, `metricType: "CATALOG"`) mapping the two verified category paths: `/category/new-arrivals/23/` (mapped to `LONG_SLEEVE_TSHIRT` and `PANTS` only - the two item types actually observed with a quoted example in the sampled listing, since this section is cross-item-type and not scoped to one category) and `/category/accessories/45/` (mapped to `BAG`, mirroring the existing Slam Jam accessories-to-`BAG` precedent).
- **No collector behavior exists for this method.** `createMarketCollector` (`src/collectors/market/index.ts`) has no branch for `CAFE24_CATEGORY_HTML` and falls through to the existing `UnsupportedMarketCollector`, so this config addition cannot cause any live collection even if invoked - verified by reading that file before making this change. `ShopifyMarketCollector`, `supportedCollectorSources()`, `verifiedRankingCollectorSources()`, and `assortmentCollectorSources()` are all unaffected (none of their method-based filters match `CAFE24_CATEGORY_HTML`).
- **No DB rows written, no live `collect:market` run performed.** All evidence in `docs/MARKET_SOURCE_AUDIT.md`'s "Rednape" section came from a small number of read-only `WebFetch` calls against public pages (`robots.txt`, `sitemap.xml`, one category page, one product page) during a prior research-only pass.
