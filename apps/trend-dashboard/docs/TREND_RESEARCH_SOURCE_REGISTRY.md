...
Docs-only follow-up to Section 10 decision 3. **No code, schema, collector, taxonomy, or ranking change** - `VisualDiffusionSourceConfig`/`VisualDiffusionReference` (Section 6) remain a design proposal with zero implementation anywhere in `src/` (confirmed via a fresh repo search this pass); these entries live only in this Markdown table. **No live scraping of Instagram was performed** - each row was confirmed real via a single read-only, unauthenticated fetch of that account's own public profile page (bio/display-name/follower-count metadata only), no login, no private API, no post-content scraping, per the same boundary Section 1 established and the user's explicit instruction for this pass.
