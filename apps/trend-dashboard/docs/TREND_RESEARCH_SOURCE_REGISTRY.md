...
3. **Instagram accounts**: ~~approve registering the 4 accounts (plus the 1 example permalink) as `MANUAL_CURATION`-tier `VisualDiffusionSourceConfig` entries~~ **Approved and registered 2026-09-14** (docs-only; see Section 11 below) - 6 accounts total: the original 4 plus 2 additional accounts found and independently bio-verified in the same pass. Two other verified-real candidates were surfaced but not approved by the user for registration (see Section 11). The 1 example permalink (`instagram.com/p/DWY-0_5kYwy/`) was not registered in this pass. **Still open**: who is the human reviewer in practice, and what triggers adding a new permalink/account.
...
## 11. Instagram `MANUAL_CURATION` registry entries (2026-09-14, approved and added)

Docs-only follow-up to Section 10 decision 3. At the time of this registry pass, **no code, schema, collector, taxonomy, or ranking change** was made and `VisualDiffusionSourceConfig`/`VisualDiffusionReference` (Section 6) still had zero implementation in `src/`; the later scaffold implementation is recorded in Section 12 below. **No live scraping of Instagram was performed** - each row was confirmed real via a single read-only, unauthenticated fetch of that account's own public profile page (bio/display-name/follower-count metadata only), no login, no private API, no post-content scraping, per the same boundary Section 1 established and the user's explicit instruction for this pass.

The user separately confirmed the first 4 accounts by handle/description before this pass began. This pass then searched for and independently bio-verified 4 additional real, currently-active candidates of the same kind (large-follower Korean fashion curation/magazine accounts posting outfit or product picks, not brand accounts, not personal-life influencer content); the user reviewed all 4 and approved 2 for registration (`@fashion_curator_seoul`, `@celeb_fashion_magazine`). The other 2 verified-real candidates (`@styleandrich.official`, 36.8K, self-described "취향기반 패션 큐레이션"; `@dailyfashion_news`, 1M, self-described "패션 뉴스 요약" - judged a weaker fit, closer to news-roundup than outfit/product-pick styling) were not approved and are **not** registered below. No unverified handle was ever added or guessed at any point.

| id | platform | handle | profileUrl | role | collectionMethod | relatedEditorialSource | note (from the account's own bio) |
|---|---|---|---|---|---|---|---|
| `IG_FASHION_PLATFORM_SEOUL` | `INSTAGRAM` | `@fashion_platform_seoul` | `https://www.instagram.com/fashion_platform_seoul/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | User-confirmed. Self-described fashion curation platform ("패플서", 234K followers); bio: "패플서입니다🫡 ... 패큐서도 놀러오시길 @fashion_curator_seoul" - posts outfit/product-focused "무물"/styling content and cross-links `@fashion_curator_seoul` as a sister account. |
| `IG_JENTESTORE` | `INSTAGRAM` | `@jentestore` | `https://www.instagram.com/jentestore/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | User-confirmed. Self-described fashion curation platform ("패션 큐레이션 플랫폼 젠테스토어", 71K followers); bio: "Art in Life, jente 패션 큐레이션 플랫폼 젠테스토어" - editorial-style curation with a linked shop. |
| `IG_XXPICK` | `INSTAGRAM` | `@_xxpick` | `https://www.instagram.com/_xxpick/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | User-confirmed. Self-described editor account ("XXp!ck \| 엑세스픽", 88K followers) posting outfit and product pick content. |
| `IG_HUMANRETROGIRL` | `INSTAGRAM` | `@humanretrogirl` | `https://www.instagram.com/humanretrogirl/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | User-confirmed. Self-described fashion magazine ("휴레걸의 패션매거진", 64K followers); bio: "휴레걸의 패션매거진 •✰.*•." - styling roundup / wishlist-style posts. |
| `IG_FASHION_CURATOR_SEOUL` | `INSTAGRAM` | `@fashion_curator_seoul` | `https://www.instagram.com/fashion_curator_seoul/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | Found and bio-verified this pass (61K followers), approved by the user. Self-described fashion curator ("패큐서"); bio: "안녕하세요. 패큐서입니다🫡 최신 패션 뉴스 @fashion_platform_seoul 패큐서 픽 꿀템은 아래 링크로" - the account `@fashion_platform_seoul`'s own bio names as its sister account, and links curated outfit/product picks to a Musinsa shopping link. |
| `IG_CELEB_FASHION_MAGAZINE` | `INSTAGRAM` | `@celeb_fashion_magazine` | `https://www.instagram.com/celeb_fashion_magazine/` | `["STYLING_REFERENCE", "CURATION"]` | `MANUAL_CURATION` | `null` | Found and bio-verified this pass (864K followers), approved by the user. Self-described digital fashion/celebrity magazine ("셀패진"); bio: "셀럽패션매거진 지금 핫한 패션·뷰티·트렌드는 셀패진에서! 쉽고 빠르게 만나는 디지털매거진" - celebrity fashion/beauty/trend picks. |

`relatedEditorialSource` is `null` for every row - none of these 6 accounts was checked against or claimed to match any registered web `EditorialSource` (the Section 5 NONLABEL web/Instagram boundary applies unchanged: the `@fashion_platform_seoul` <-> `@fashion_curator_seoul` cross-link is between two Instagram accounts, not a web editorial source, so it belongs in the human-written `note` field, never in `relatedEditorialSource`).

**Standard disclaimer for every entry in this table, per this project's existing pattern**: 사람이 직접 선별한 참고 자료 · 직접 증거 아님, 판매량 아님. None of these 6 accounts feeds, or is proposed to feed, any automated collector, `EditorialMention`, `attribute-bundle-service.ts`, or `MarketRankingSnapshot` - this table is a human-reviewed reference registry only, exactly as scoped in Section 6's constraints.

**What changed vs. what did not (this section)**: New - this table, plus the Section 10 decision 3 status update above, and matching pointer updates in `CURRENT_STATE.md`/`NEXT_PRIORITIES.md`. Unchanged - everything else in this document, `editorialSourceConfigs`, `marketSources`, any collector, schema, taxonomy, or ranking logic. No live Editorial refresh was triggered by this pass (the corpus growth visible in a live DB check during this pass - see `CURRENT_STATE.md` - matches the independently scheduled 2026-09-14 08:30 KST weekly refresh, not anything this pass did).

## 12. Visual Diffusion code scaffold (2026-09-14, implemented empty)

Later same-day implementation of Section 6/7's already-reviewed boundary:

- The six approved Section 11 accounts are represented in `src/config/visual-diffusion-sources.ts` as `MANUAL_CURATION` entries only.
- `src/config/visual-diffusion-references.ts` implements the typed human-review record and exact-`specificItem` lookup. Its exported reference array is empty. No Instagram post/permalink/caption/mood/item assignment has been added without explicit user approval of that exact post.
- `src/components/VisualDiffusionReferences.tsx` is a permalink-only, separately labelled UI lane with the permanent Section 7 disclaimer. It fetches, hotlinks, caches, and transforms no Instagram image. `CurrentSignalHero` calls it after Editorial Visual Context; with the empty list it returns `null` and displays nothing.
- The source/reference modules have no imports from collectors, Prisma, extraction, bundle ranking, or Market code, and no such modules import them. No schema, collector, taxonomy, ranking, or evidence-semantics change was made.

Validation from `apps/trend-dashboard/`: typecheck, smoke test, and production build passed; rendered desktop/mobile checks confirmed the empty lane stays absent. The next reference-level action remains gated on the user's approval of an exact permalink.
