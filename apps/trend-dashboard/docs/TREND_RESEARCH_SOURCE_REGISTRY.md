...
1. **Coverchord**: approve the small, isolated, config-only follow-up (add to `marketSources` + a collector category mapping, mirroring Slam Jam/Stussy) - genuinely ready now, gated only on explicit approval, not on P0.
