...
- Order-data memory: `발주데이터/markdown/README.md` indexes 153 converted Markdown documents from `발주데이터` covering 2023SS, 2024SS/FW, 2025SS/FW, 2026FW, and related guide/manual workbooks. Use this Markdown index first when searching historical order lists, product MAPs, line sheets, reviews, CAD report text, SKU production guides, inbound/sales plans, or sourcing/order reference data. The Markdown files preserve original ZIP/member paths and sheet metadata; large sheets prioritize the first non-empty rows/columns, so consult the original workbook when a truncated sheet requires exact row-level data.
